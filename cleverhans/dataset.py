...
class SVHN(Dataset):
    ...
    def __init__(self, train_start=0, train_end=73257, test_start=0,
                 test_end=26032):
        super(SVHN, self).__init__(locals())
...
def download_and_parse_svhn_file(file_name, datadir=None, force=False):
    url = os.path.join('http://ufldl.stanford.edu/housenumbers/', file_name)
    file_name = maybe_download_file(url, datadir=datadir, force=force)
...
def data_svhn(datadir=tempfile.gettempdir(), train_start=0,
              train_end=73257, test_start=0, test_end=26032):
    """
    Load and preprocess MNIST dataset
    :param datadir: path to folder where data should be stored
    :param train_start: index of first training set example
    ...
    train_set = download_and_parse_svhn_file('train_32x32.mat', datadir=datadir)
